...
ensure_json_exists(std_class_path)
...
while True:
    ...
    if cv2.waitKey(1) & 0xff == ord('q'):
        # send_message()
        with open(Jc1_path,'r') as f:
          dataX = json.load(f)
          for x in dataX:
                  Jc1Present.append(dataX[x]['name'])
          for x in Jc1Def:
              if x not in Jc1Present:
                  Jc1Absent.append(x)
          print(Jc1Absent)
          print(Jc1Present)


          text_content = f"\nGood Morning Mr Sammy!\nPresent: {len(Jc1Present)}\nAbsent: {len(Jc1Absent)}\n"
          text_content += "Here are the people who are present today!\n"

          for student in Jc1Present:
              text_content += f"\t{student}\n"

          text_content += "\nHere are the people who are absent today!\n"

          for student in Jc1Absent:
              text_content += f"\t{student}\n"

          file_path = 'students.txt'
          with open('jc1.txt', 'a') as file:
              file.write(text_content)
        # LoadPresentAbsent(Jc1_path,Jc1Present,Jc1Absent)
        from twilio.rest import Client
